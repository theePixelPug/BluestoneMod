...
public class BluestoneComparatorBlock extends AbstractBluestoneGateBlock implements BlockEntityProvider {
    ...
    protected void updatePowered(World world, BlockPos pos, BlockState state) {
        if (!world.getBlockTickScheduler().isTicking(pos, this)) {
            ...
            if (i != j || (Boolean)state.get(BLUESTONE_POWERED) != this.hasPower(world, pos, state)) {
    ...
    private void update(World world, BlockPos pos, BlockState state) {
        ...
        if (j != i || state.get(MODE) == ComparatorMode.COMPARE) {
            ...
            boolean bl2 = (Boolean)state.get(BLUESTONE_POWERED);
            if (bl2 && !bl) {
    ...
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        builder.add(new Property[]{FACING, MODE, BLUESTONE_POWERED});
    }
