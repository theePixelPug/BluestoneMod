...
public class BluestoneDetectorRailBlock extends AbstractRailBlock {
    public static final EnumProperty<RailShape> SHAPE;
    public static final BooleanProperty BLUESTONE_POWERED;
    private static final int SCHEDULED_TICK_DELAY = 20;

    public BluestoneDetectorRailBlock(AbstractBlock.Settings settings) {
        super(true, settings);
        this.setDefaultState((BlockState)((BlockState)((BlockState)((BlockState)this.stateManager.getDefaultState()).with(BLUESTONE_POWERED, false)).with(SHAPE, RailShape.NORTH_SOUTH)).with(WATERLOGGED, false));
    }
    ...
    public int getWeakBluestonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        return (Boolean)state.get(BLUESTONE_POWERED) ? 15 : 0;
    }
    ...
    private void updatePoweredStatus(World world, BlockPos pos, BlockState state) {
        if (this.canPlaceAt(state, world, pos)) {
            ...
            boolean bl2 = false;
            List<AbstractMinecartEntity> list = this.getCarts(world, pos, AbstractMinecartEntity.class, (entity) -> {
                return true;
            });
            if (!list.isEmpty()) {
                bl2 = true;
            ...
            if (bl2 && !bl) {
                blockState = (BlockState)state.with(BLUESTONE_POWERED, true);
            ...
            if (!bl2 && bl) {
                blockState = (BlockState)state.with(BLUESTONE_POWERED, false);
                world.setBlockState(pos, blockState, 3);
    ...
    public int getComparatorOutput(BlockState state, World world, BlockPos pos) {
        if ((Boolean)state.get(BLUESTONE_POWERED)) {
            List<CommandBlockMinecartEntity> list = this.getCarts(world, pos, CommandBlockMinecartEntity.class, (cart) -> {
                return true;
            });
            if (!list.isEmpty()) {
