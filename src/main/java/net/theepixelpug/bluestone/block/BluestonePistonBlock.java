...
public class BluestonePistonBlock
        extends FacingBlock {
    ...
    public BluestonePistonBlock(boolean sticky, AbstractBlock.Settings settings) {
        super(settings);
    ...
    private void tryMove(World world, BlockPos pos, BlockState state) {
        ...
        if (bl && !state.get(EXTENDED).booleanValue()) {
            ...
            BluestonePistonBlockEntity pistonBlockEntity;
            BlockEntity blockEntity;
            BlockPos blockPos = pos.offset(direction, 2);
            BlockState blockState = world.getBlockState(blockPos);
            int i = 1;
            if (blockState.isOf(ModBlocks.BLUESTONE_MOVING_PISTON) && blockState.get(FACING) == direction && (blockEntity = world.getBlockEntity(blockPos)) instanceof BluestonePistonBlockEntity && (pistonBlockEntity = (BluestonePistonBlockEntity)blockEntity).isExtending() && (pistonBlockEntity.getProgress(0.0f) < 0.5f || world.getTime() == pistonBlockEntity.getSavedWorldTime() || ((ServerWorld)world).isInBlockTick())) {
                i = 2;
            }
    ...
    private boolean shouldExtend(World world, BlockPos pos, Direction pistonFace) {
        for (Direction direction : Direction.values()) {
            if (direction == pistonFace || !((WorldMixinInterface)world).isEmittingBluestonePower(pos.offset(direction), direction)) continue;
            return true;
        }
        if (((WorldMixinInterface)world).isEmittingBluestonePower(pos, Direction.DOWN)) {
            return true;
        }
        BlockPos blockPos = pos.up();
        for (Direction direction2 : Direction.values()) {
            if (direction2 == Direction.DOWN || !((WorldMixinInterface)world).isEmittingBluestonePower(blockPos.offset(direction2), direction2)) continue;
            return true;
        }
        return false;
    }
    ...
    public boolean onSyncedBlockEvent(BlockState state, World world, BlockPos pos, int type, int data) {
        ...
        if (type == 0) {
            ...
            BlockState blockState = (BlockState)((BlockState)ModBlocks.BLUESTONE_MOVING_PISTON.getDefaultState().with(BluestonePistonExtensionBlock.FACING, direction)).with(BluestonePistonExtensionBlock.TYPE, this.sticky ? PistonType.STICKY : PistonType.DEFAULT);
            world.setBlockState(pos, blockState, Block.NO_REDRAW | Block.FORCE_STATE);
            world.addBlockEntity(BluestonePistonExtensionBlock.createBlockEntityPiston(pos, blockState, (BlockState)this.getDefaultState().with(FACING, Direction.byId(data & 7)), direction, false, true));
            ...
            if (this.sticky) {
                ...
                boolean bl2 = false;
                if (blockState2.isOf(ModBlocks.BLUESTONE_MOVING_PISTON) && (blockEntity2 = world.getBlockEntity(blockPos)) instanceof BluestonePistonBlockEntity && (pistonBlockEntity = (BluestonePistonBlockEntity)blockEntity2).getFacing() == direction && pistonBlockEntity.isExtending()) {
                    pistonBlockEntity.finish();
                    bl2 = true;
                }
                if (!bl2) {
                    if (type == 1 && !blockState2.isAir() && BluestonePistonBlock.isMovable(blockState2, world, blockPos, direction.getOpposite(), false, direction) && (blockState2.getPistonBehavior() == PistonBehavior.NORMAL || blockState2.isOf(ModBlocks.BLUESTONE_PISTON) || blockState2.isOf(ModBlocks.BLUESTONE_STICKY_PISTON))) {
    ...
    public static boolean isMovable(BlockState state, World world, BlockPos pos, Direction direction, boolean canBreak, Direction pistonDir) {
        ...
        if (state.isOf(ModBlocks.BLUESTONE_PISTON) || state.isOf(ModBlocks.BLUESTONE_STICKY_PISTON)) {
            if (state.get(EXTENDED).booleanValue()) {
                return false;
            }
        } else {
    ...
    private boolean move(World world, BlockPos pos, Direction dir, boolean retract) {
        ...
        if (!retract && world.getBlockState(blockPos).isOf(ModBlocks.BLUESTONE_PISTON_HEAD)) {
        ...
        for (k = list3.size() - 1; k >= 0; --k) {
            ...
            BluestonePistonBlock.dropStacks(blockState, world, blockPos3, blockEntity);
        ...
        for (k = list.size() - 1; k >= 0; --k) {
            ...
            BlockState blockState3 = (BlockState)ModBlocks.BLUESTONE_MOVING_PISTON.getDefaultState().with(FACING, dir);
            world.setBlockState(blockPos3, blockState3, Block.NO_REDRAW | Block.MOVED);
            world.addBlockEntity(BluestonePistonExtensionBlock.createBlockEntityPiston(blockPos3, blockState3, (BlockState)list2.get(k), dir, retract, false));
            blockStates[j++] = blockState;
        }
        if (retract) {
            PistonType pistonType = this.sticky ? PistonType.STICKY : PistonType.DEFAULT;
            BlockState blockState4 = (BlockState)((BlockState)ModBlocks.BLUESTONE_PISTON_HEAD.getDefaultState().with(BluestonePistonHeadBlock.FACING, dir)).with(BluestonePistonHeadBlock.TYPE, pistonType);
            BlockState blockState = (BlockState)((BlockState)ModBlocks.BLUESTONE_MOVING_PISTON.getDefaultState().with(BluestonePistonExtensionBlock.FACING, dir)).with(BluestonePistonExtensionBlock.TYPE, this.sticky ? PistonType.STICKY : PistonType.DEFAULT);
            map.remove(blockPos);
            world.setBlockState(blockPos, blockState, Block.NO_REDRAW | Block.MOVED);
            world.addBlockEntity(BluestonePistonExtensionBlock.createBlockEntityPiston(blockPos, blockState, blockState4, dir, true, true));
        }
        ...
        if (retract) {
            world.updateNeighborsAlways(blockPos, ModBlocks.BLUESTONE_PISTON_HEAD);
        }
        return true;
    }
