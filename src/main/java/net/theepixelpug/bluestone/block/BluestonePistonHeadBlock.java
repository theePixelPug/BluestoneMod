...
public class BluestonePistonHeadBlock
        extends FacingBlock {
    ...
    private static final VoxelShape[] SHORT_HEAD_SHAPES = BluestonePistonHeadBlock.getHeadShapes(true);
    private static final VoxelShape[] HEAD_SHAPES = BluestonePistonHeadBlock.getHeadShapes(false);

    private static VoxelShape[] getHeadShapes(boolean shortHead) {
        return (VoxelShape[])Arrays.stream(Direction.values()).map(direction -> BluestonePistonHeadBlock.getHeadShape(direction, shortHead)).toArray(VoxelShape[]::new);
    }
    ...
    public BluestonePistonHeadBlock(AbstractBlock.Settings settings) {
        super(settings);
    ...
    private boolean isAttached(BlockState headState, BlockState pistonState) {
        Block block = headState.get(TYPE) == PistonType.DEFAULT ? ModBlocks.BLUESTONE_PISTON : ModBlocks.BLUESTONE_STICKY_PISTON;
        return pistonState.isOf(block) && pistonState.get(BluestonePistonBlock.EXTENDED) != false && pistonState.get(FACING) == headState.get(FACING);
    }
    ...
    public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        ...
        return this.isAttached(state, blockState) || blockState.isOf(ModBlocks.BLUESTONE_MOVING_PISTON) && blockState.get(FACING) == state.get(FACING);
    }
    ...
    public ItemStack getPickStack(BlockView world, BlockPos pos, BlockState state) {
        return new ItemStack(state.get(TYPE) == PistonType.STICKY ? ModBlocks.BLUESTONE_STICKY_PISTON : ModBlocks.BLUESTONE_PISTON);
    }
