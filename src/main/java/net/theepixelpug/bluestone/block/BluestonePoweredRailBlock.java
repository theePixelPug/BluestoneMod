...
public class BluestonePoweredRailBlock extends AbstractRailBlock {
    public static final EnumProperty<RailShape> SHAPE;
    public static final BooleanProperty BLUESTONE_POWERED;

    public BluestonePoweredRailBlock(AbstractBlock.Settings settings) {
        super(true, settings);
        this.setDefaultState((BlockState)((BlockState)((BlockState)((BlockState)this.stateManager.getDefaultState()).with(SHAPE, RailShape.NORTH_SOUTH)).with(BLUESTONE_POWERED, false)).with(WATERLOGGED, false));
    }
    ...
    protected boolean isPoweredByOtherRails(World world, BlockPos pos, boolean bl, int distance, RailShape shape) {
        ...
        if (!blockState.isOf(this)) {
            ...
            if (shape == RailShape.EAST_WEST && (railShape == RailShape.NORTH_SOUTH || railShape == RailShape.ASCENDING_NORTH || railShape == RailShape.ASCENDING_SOUTH)) {
                ...
                return false;
            } else if ((Boolean)blockState.get(BLUESTONE_POWERED)) {
                return ((WorldMixinInterface)world).isReceivingBluestonePower(pos) ? true : this.isPoweredByOtherRails(world, pos, blockState, bl, distance + 1);
            } else {
                return false;
            }
        }
    ...
    protected void updateBlockState(BlockState state, World world, BlockPos pos, Block neighbor) {
        ...
        boolean bl2 = ((WorldMixinInterface)world).isReceivingBluestonePower(pos) || this.isPoweredByOtherRails(world, pos, state, true, 0) || this.isPoweredByOtherRails(world, pos, state, false, 0);
        if (bl2 != bl) {
            world.setBlockState(pos, (BlockState)state.with(BLUESTONE_POWERED, bl2), 3);
    ...
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        builder.add(new Property[]{SHAPE, BLUESTONE_POWERED, WATERLOGGED});
    }
