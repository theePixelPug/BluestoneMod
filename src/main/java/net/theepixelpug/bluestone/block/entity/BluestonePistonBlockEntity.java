...
public class BluestonePistonBlockEntity
        extends BlockEntity {
    ...
    public BluestonePistonBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEnities.BLUESTONE_PISTON, pos, state);
    }

    public BluestonePistonBlockEntity(BlockPos pos, BlockState state, BlockState pushedBlock, Direction facing, boolean extending, boolean source) {
        this(pos, state);
    ...
    private BlockState getHeadBlockState() {
        if (!this.isExtending() && this.isSource() && this.pushedBlock.getBlock() instanceof BluestonePistonBlock) {
            return (BlockState)((BlockState)((BlockState) ModBlocks.BLUESTONE_PISTON_HEAD.getDefaultState().with(BluestonePistonHeadBlock.SHORT, this.progress > 0.25f)).with(BluestonePistonHeadBlock.TYPE, this.pushedBlock.isOf(ModBlocks.BLUESTONE_STICKY_PISTON) ? PistonType.STICKY : PistonType.DEFAULT)).with(BluestonePistonHeadBlock.FACING, this.pushedBlock.get(BluestonePistonBlock.FACING));
        }
        return this.pushedBlock;
    }

    private static void pushEntities(World world, BlockPos pos, float f, BluestonePistonBlockEntity blockEntity) {
        ...
        Box box = BluestonePistonBlockEntity.offsetHeadBox(pos, voxelShape.getBoundingBox(), blockEntity);
        ...
        for (Entity entity : list) {
            ...
            while (!(!iterator.hasNext() || (box3 = Boxes.stretch(BluestonePistonBlockEntity.offsetHeadBox(pos, box2 = iterator.next(), blockEntity), direction, d)).intersects(box4 = entity.getBoundingBox()) && (i = Math.max(i, BluestonePistonBlockEntity.getIntersectionSize(box3, direction, box4))) >= d)) {
            }
            if (i <= 0.0) continue;
            i = Math.min(i, d) + 0.01;
            BluestonePistonBlockEntity.moveEntity(direction, entity, i, direction);
            if (blockEntity.extending || !blockEntity.source) continue;
            BluestonePistonBlockEntity.push(pos, entity, direction, d);
        }
    }
    ...
    private static void moveEntitiesInHoneyBlock(World world, BlockPos pos, float f, BluestonePistonBlockEntity blockEntity) {
        ...
        Box box = BluestonePistonBlockEntity.offsetHeadBox(pos, new Box(0.0, d, 0.0, 1.0, 1.5000000999999998, 1.0), blockEntity);
        double e = f - blockEntity.progress;
        List<Entity> list = world.getOtherEntities(null, box, entity -> BluestonePistonBlockEntity.canMoveEntity(box, entity));
        for (Entity entity2 : list) {
            BluestonePistonBlockEntity.moveEntity(direction, entity2, e, direction);
        }
    }
    ...
    private static Box offsetHeadBox(BlockPos pos, Box box, BluestonePistonBlockEntity blockEntity) {
    ...
    private static void push(BlockPos pos, Entity entity, Direction direction, double amount) {
        ...
        if (box.intersects(box2 = VoxelShapes.fullCube().getBoundingBox().offset(pos)) && Math.abs((d = BluestonePistonBlockEntity.getIntersectionSize(box2, direction2 = direction.getOpposite(), box) + 0.01) - (e = BluestonePistonBlockEntity.getIntersectionSize(box2, direction2, box.intersection(box2)) + 0.01)) < 0.01) {
            d = Math.min(d, amount) + 0.01;
            BluestonePistonBlockEntity.moveEntity(direction, entity, d, direction2);
        }
    }
    ...
    public void finish() {
        if (this.world != null && (this.lastProgress < 1.0f || this.world.isClient)) {
            ...
            this.markRemoved();
            if (this.world.getBlockState(this.pos).isOf(ModBlocks.BLUESTONE_MOVING_PISTON)) {
    ...
    public static void tick(World world, BlockPos pos, BlockState state, BluestonePistonBlockEntity blockEntity) {
        ...
        if (blockEntity.lastProgress >= 1.0f) {
            ...
            if (world.getBlockState(pos).isOf(ModBlocks.BLUESTONE_MOVING_PISTON)) {
        ...
        BluestonePistonBlockEntity.pushEntities(world, pos, f, blockEntity);
        BluestonePistonBlockEntity.moveEntitiesInHoneyBlock(world, pos, f, blockEntity);
    ...
    public VoxelShape getCollisionShape(BlockView world, BlockPos pos) {
        VoxelShape voxelShape = !this.extending && this.source && this.pushedBlock.getBlock() instanceof BluestonePistonBlock ? ((BlockState)this.pushedBlock.with(BluestonePistonBlock.EXTENDED, true)).getCollisionShape(world, pos) : VoxelShapes.empty();
        Direction direction = field_12205.get();
        if ((double)this.progress < 1.0 && direction == this.getMovementDirection()) {
            return voxelShape;
        }
        BlockState blockState = this.isSource() ? (BlockState)((BlockState)ModBlocks.BLUESTONE_PISTON_HEAD.getDefaultState().with(BluestonePistonHeadBlock.FACING, this.facing)).with(BluestonePistonHeadBlock.SHORT, this.extending != 1.0f - this.progress < 0.25f) : this.pushedBlock;
